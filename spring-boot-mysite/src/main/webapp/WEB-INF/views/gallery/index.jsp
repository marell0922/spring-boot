		...
		<div id="content">
			<div id="gallery">
				<div>
					<h1>갤러리</h1>
					<c:if test="${not empty authUser && authUser.role=='ADMIN' }">
						<a href="${pageContext.servletContext.contextPath }/update" id="upload-image">이미지 올리기</a>
					</c:if>
				</div>
				<ul>
				<c:forEach items="${list }" var="vo">
					<li>
						<a	href="${pageContext.request.contextPath }${vo.image }"
							class="image"
							data-lightbox="gallery"
							style="background-image:url('${pageContext.request.contextPath }${vo.image }')">&nbsp;</a>
						<c:if test="${not empty authUser && authUser.role=='ADMIN' }">
							<a	href="${pageContext.servletContext.contextPath }/${vo.no}" class="del-button" title="삭제">삭제</a>
						</c:if>
					</li>
				</c:forEach>
			...
			<c:if test="${not empty authUser && authUser.role=='ADMIN' }">
				<div id="dialog-upload-form" title="이미지 업로드" style="display: none">
					...
					<form action="${pageContext.request.contextPath }/gallery/update" method="post" enctype="multipart/form-data">
						<label>코멘트</label> <input type="text" id="input-comments" name="comment" value="">
						<label>이미지</label> <input type="file" id="input-file" name="update-image"> 
